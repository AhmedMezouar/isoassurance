<template>
  <div>
    ...
    <div
      class="container mx-auto p-4 md:w-11/12 grid grid-cols-1 gap-8 billing-mobile"
    >
      <!-- Information card-->
      <div>
        <h1 class="text-xl font-semibold">
          {{ otherRef.name + " " + otherRef.surname ? otherRef.surname : "" }}
        </h1>
        <span>CARD |</span>
        <span class="text-teal-500">{{
          otherRef.uuid ? otherRef.uuid : ""
        }}</span>
      </div>
...
const searchAccount = async (uuid) => {
  const account = await fetch(
    `https://backend.assuranceiso.com/api/account/findByUuid/${uuid}`
  ).then((r) => r.json());
